    ...
    <View className="flex-1 bg-[#0a0a0a]">
      <ScrollView className="p-5" showsVerticalScrollIndicator={false}>
        {isLoading ? (
          ...
          data.map((shield) => (
            <View key={shield.id} className="mt-2 items-center pb-10">
              <Image
                source={{ uri: shield.image }}
              ...
              <View className="bg-gray-900/80 p-5 rounded-2xl w-full border border-yellow-900/30 shadow-lg">
                ...
                <View className="space-y-4">
                  <View className="flex-row justify-between border-b border-gray-800 pb-3">
                    ...
                    <Text className="text-red-400 font-serif text-right w-2/3">{shield.category}</Text>
                  </View>
                  <View className="flex-row justify-between border-b border-gray-800 pb-3">
                    <Text className="text-gray-400 font-semibold uppercase tracking-wider text-xs">Weight</Text>
                    <Text className="text-blue-400 font-serif font-bold">{shield.weight}</Text>
                  </View>
                </View>
                {shield.attack && shield.attack.length > 0 && (
                ...
                {shield.defence && shield.defence.length > 0 && (
                ...
                {shield.requiredAttributes && shield.requiredAttributes.length > 0 && (
                  <View className="mt-4 bg-black/40 p-4 rounded-xl border border-gray-800">
                    ...
                    {shield.requiredAttributes.map((r, i) => (
                      <View key={i} className="flex-row justify-between py-1">
                ...
                {shield.scalesWith && shield.scalesWith.length > 0 && (
                  <View className="mt-4 bg-black/40 p-4 rounded-xl border border-gray-800">
